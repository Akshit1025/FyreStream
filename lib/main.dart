    ...
    switch (urlType) {
      case UrlType.spotifyTrack:
        ExternalMediaImporter.sfyMediaImporter(sharedMediaFiles[0].path)
            .then((value) async {
          if (value != null) {
            await fyrestreamPlayerCubit.fyrestreamPlayer
...
Future<void> importItems(String path) async {
  bool _res = await FyreStreamFileManager.importMediaItem(path);
  if (_res) {
    SnackbarService.showMessage("Media Item Imported");
  } else {
    _res = await FyreStreamFileManager.importPlaylist(path);
    if (_res) {
...
class _MyAppState extends State<MyApp> {
  ...
  void dispose() {
    _intentSub.cancel();
    fyrestreamPlayerCubit.fyrestreamPlayer.audioPlayer.dispose();
    fyrestreamPlayerCubit.close();
    super.dispose();
  }
  @override
      ...
      providers: [
        ...
        BlocProvider(
          create: (context) => CurrentPlaylistCubit(
              fyrestreamDBCubit: context.read<FyreStreamDBCubit>()),
          lazy: false,
        ),
        BlocProvider(
          create: (context) =>
