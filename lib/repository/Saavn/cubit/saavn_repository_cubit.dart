...
class SaavnSearchRepositoryCubit extends Cubit<SaavnRepositoryState> {
  ...
  Future<void> initializeAccessTokenWithDebounce() async {
    if (accessSpotifyToken != null) {
      EasyDebounce.debounce(
        'initializeTokenDebounce',
        const Duration(milliseconds: 59 * 60 * 1000),
        () async {
          initializeAccessToken();
  ...
  Future<void> fetchSearchResultsFromSaavn(
    String query, {
    bool spotify = true,
  }) async {
    ...
    for (int i = 0; i < search_queries.length; i++) {
      ...
      searchResultsList += fromSaavnSongMapList2MediaItemList(
        trendingResults["songs"],
      );
    }
    searchResultsList.toSet().toList();
    emit(
      SaavnRepositoryState(mediaItems: searchResultsList, albumName: "Search"),
    );
  }
    ...
    if (accessSpotifyToken != null) {
      ...
      if (_spotifyList.length > 0) {
        for (int k = 0; k < _spotifyList.length; k++) {
          ...
          String _artists = "";
          (await _spotifyList[k]["track"]["artists"] as List<dynamic>).forEach((
            element,
          ) {
            _artists = "${_artists} ${element["name"]}";
          });
          final saavnSearchResult = await SaavnAPI().fetchSongSearchResults(
            searchQuery: "${_title} ${_artists}}",
            count: 1,
          );
          final searchResultsList = fromSaavnSongMapList2MediaItemList(
            saavnSearchResult["songs"],
          );
          if (searchResultsList.isNotEmpty) {
            importFromSpotifyState.add(
              ImportPlaylistState(
            ...
            _mediaDBCubit.addMediaItemToPlaylist(
              searchResultsList[0],
