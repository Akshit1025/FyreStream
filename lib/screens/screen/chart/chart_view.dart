...
class _ChartScreenState extends State<ChartScreen> {
  ...
  Future<ChartModel?> getChart() async {
    return await FyreStreamDBService.getChart(widget.chartName);
  }
    ...
    return Scaffold(
      body: FutureBuilder(
        ...
        builder: (context, state) {
          if (state.connectionState == ConnectionState.waiting || state.data == null) {
            return const Center(
              child: SizedBox(
                  height: 50,
                  width: 50,
            ...
            Container(
              decoration: BoxDecoration(
                image: DecorationImage(
                    image: NetworkImage(state.chartItems!.first.imageUrl.toString()), fit: BoxFit.cover),
              ),
            ),
            Positioned.fill(
                child: Container(
