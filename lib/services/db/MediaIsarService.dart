...
class MediaIsarDBService {
  ...
  Future<void> reorderItemPositionInPlaylist(
    ...
    if (_mediaPlaylistDB != null) {
      if (_mediaPlaylistDB.mediaRanks.length > old_idx &&
        ...
        List<int> _rankList = _mediaPlaylistDB.mediaRanks.toList(
          growable: true,
        );
  ...
  Future<bool> isMediaLiked(MediaItemDB mediaItemDB) async {
    ...
    if (_mediaItemDB != null) {
      return (isarDB.mediaPlaylistDBs.getSync(
            MediaPlaylistDB(playlistName: "Liked").isarId,
          ))?.mediaItems.contains(_mediaItemDB) ??
          true;
    } else {
      return false;
    }
  ...
  Future<void> setPlaylistItemsRank(
    ...
    Isar isarDB = await db;
    MediaPlaylistDB? _mediaPlaylistDB = isarDB.mediaPlaylistDBs.getSync(
      mediaPlaylistDB.isarId,
    );
    if (_mediaPlaylistDB != null &&
