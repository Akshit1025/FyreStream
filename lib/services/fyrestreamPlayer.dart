...
class FyreStreamMusicPlayer extends BaseAudioHandler
  ...
  Future<String?> latestYtLink(String id) async {
    ...
    if (vidInfo != null) {
      if ((DateTime.now().millisecondsSinceEpoch ~/ 1000) + 350 >
          vidInfo.expireAt) {
        log("Link expired for vidId: $id", name: "fyrestreamPlayer");
  ...
  Future<AudioSource> getAudioSource(MediaItem mediaItem) async {
    final _down = await FyreStreamDBService.getDownloadDB(
        mediaItem2MediaItemModel(mediaItem));
    if (_down != null) {
      log("Playing Offline", name: "fyrestreamPlayer");
      ...
      isOffline.add(false);
      log("Playing online", name: "fyrestreamPlayer");
  ...
  Future<void> playMediaItem(MediaItem mediaItem, {bool doPlay = true}) async {
    ...
    getLinkOperation = CancelableOperation.fromFuture(
      ...
      onCancel: () {
        log("skipping....", name: "fyrestreamPlayer");
        return;
      },
    );
    ...
    getLinkOperation.then((value) async {
      if (value != null) {
        try {
          ...
          isLinkProcessing.add(false);
          log("Error: $e", name: "fyrestreamPlayer");
