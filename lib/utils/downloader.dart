...
class FyreStreamDownloader {
  ...
  static Future<bool> alreadyDownloaded(MediaItemModel song) async {
    final tempDB = await FyreStreamDBService.getDownloadDB(song);
    if (tempDB != null) {
      ...
      if (isExist) {
        ...
        await FyreStreamDBService.removeDownloadDB(song);
      }
    }
    ...
    if (!(await alreadyDownloaded(song))) {
      try {
        ...
        log("Failed to add ${song.title} to download queue",
            error: e, name: "FyreStreamDownloader");
      }
    } else {
  ...
  static Future<void> songTagger(MediaItemModel song, String filePath) async {
    ...
    log("Tagging ${song.title} by ${song.artist}", name: "FyreStreamDownloader");
    // final imgPath =
    //     await downloadFile(song.artUri.toString(), "${song.id}.jpg");
    // log("Image downloaded for ${imgPath}", name: "FyreStreamDownloader");
    try {
  ...
  static Future<String?> latestYtLink(String id) async {
    final vidInfo = await FyreStreamDBService.getYtLinkCache(id);
    if (vidInfo != null) {
      if ((DateTime.now().millisecondsSinceEpoch ~/ 1000) + 350 >
          vidInfo.expireAt) {
        log("Link expired for vidId: $id", name: "FyreStreamDownloader");
        return await refreshYtLink(id);
      } else {
        log("Link found in cache for vidId: $id", name: "FyreStreamDownloader");
        String kurl = vidInfo.lowQURL!;
        await FyreStreamDBService.getSettingStr(GlobalStrConsts.ytDownQuality)
            .then((value) {
          if (value != null) {
            if (value == "High") {
      ...
      log("No cache found for vidId: $id", name: "FyreStreamDownloader");
      return await refreshYtLink(id);
    }
  }
