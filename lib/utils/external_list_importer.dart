...
import 'package:fyrestream/repository/MixedAPI/mixed_api.dart';
        ...
        await for (var video in tempStrm) {
          {
            ...
            FyreStreamDBService.addMediaItem(MediaItem2MediaItemDB(item),
      ...
      try {
        ...
        if (tracks.isNotEmpty) {
          for (var (e as Map) in tracks) {
            ...
            if (title.isNotEmpty) {
              final mediaItem = await MixedAPI().getTrackMixed("$title $artists".trim().toLowerCase());
              if (mediaItem != null) {
                FyreStreamDBService.addMediaItem(MediaItem2MediaItemDB(mediaItem), MediaPlaylistDB(playlistName: playlistTitle));
                yield ImporterState(
