...
class FyreStreamFileManager {
  static Future<bool> isPlaylistExists(String playlistName) async {
    // check if playlist exists
    final _list = await FyreStreamDBService.getPlaylists4Library();
  ...
  static Future<String?> exportPlaylist(String playlistName) async {
    ...
    final mediaPlaylistDB = await FyreStreamDBService.getPlaylist(playlistName);
    if (mediaPlaylistDB != null) {
      try {
        List<MediaItemDB>? playlistItems =
        await FyreStreamDBService.getPlaylistItems(mediaPlaylistDB);
      ...
      if (playlistMap != null && playlistMap.isNotEmpty) {
        try {
          ...
          for (final mediaItemMap in playlistMap['mediaItems']) {
            ...
            await FyreStreamDBService.addMediaItem(mediaItemDB, mediaPlaylistDB);
